...
public class TextMessage implements Serializable, KVMessage {
	...
	private static final char MAX_KEY = 20;	
	private static final char MAX_VALUE = 20000;	
	private static final char MAX_STATUS = 1;	
	...
	public TextMessage(byte[] bytes) {
		...
		if (msgBytes.length > (MAX_STATUS + MAX_KEY)) {
     ...
     */
	public TextMessage(String key, String value, StatusType status) {
		this.key = key;
		this.value = value;
		this.status = status;
		// Serialize the data
